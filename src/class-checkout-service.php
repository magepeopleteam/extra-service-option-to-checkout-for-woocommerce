...
if ( ! class_exists( 'Meps_Checkout_Service' ) ) {
	class Meps_Checkout_Service extends MEPS {
		...
		public function __construct() {
			...
			add_action( 'woocommerce_review_order_before_order_total', array( $this, 'meps_extra_service_before_total' ) );
			add_action( 'woocommerce_cart_totals_before_order_total', array( $this, 'meps_extra_service_before_total' ) );
				...
				<div class="meps-extra-service-content">
					...
					if ( $meps_fields ) :
						...
						foreach ( $meps_fields['service'] as $field ) :
							...
							if ( ! $this->helper->meps_cart_meet_the_service_condition( $field['condition'] ) ) {
								continue;
							}
							?>
